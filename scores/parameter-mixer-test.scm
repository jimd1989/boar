(import boar-param boar-mixer boar-noise boar-slice boar-osc srfi-4 srfi-18)

; audio handle
(audio-start! SIO-0)

;(/ (u8vector-length (dsp-buffer-bytes
;  (condition-variable-specific (audio-handle-condition-variable SIO-0)))) 4)

; 2 outputs, 2 inputs, buffer len 1184
(define mix (mixer-from-lengths 2 2 1184))
(mixer-master-volume-set!   mix     0.2)
(mixer-master-balance-set!  mix 0   1.0)
(mixer-master-balance-set!  mix 1   1.0)

; input channel 1 → left ear
(mixer-channel-volume-set!  mix 0   1.0)
(mixer-channel-balance-set! mix 0 0 1.0)
(mixer-channel-balance-set! mix 0 1 0.0)

; input channel 2 → right ear
; TO INVESTIGATE:
; - still slight click
; - double-setting a value: 0 then 0, gets weird buzz
(mixer-channel-volume-set!  mix 1   1.0)
(mixer-channel-balance-set! mix 1 0 0.0)
(mixer-channel-balance-set! mix 1 1 1.0)

; audio handle state = mixer
(audio-data-set! SIO-0 mix)

; noise
(define no (noise-from-length 1184))

; sine
(define sine (make-sine-wavetable))
(define o2 (osc-from-wavetable sine 48000))

; scratch
(osc-freq-set! o2 220.0)
; 1 + out-ch is incorrect
; should be... (*
;                 (+ 1 out-ch) ← vol    bal_0 … bal_n
;                 (+ 1 in-ch)  ← master in_0  … in_n
;                 )

; main audio loop
(audio-f-set! SIO-0
  (lambda (u8 x n)
    (if (> n 0)
      (let ((ch1-sl (mixer-channel-slice mix 0))
            (ch2-sl (mixer-channel-slice mix 1)))
        ; generate noise
        (fill-white-noise! no)
        ;(f32slice-copy-from-f32vector ch1-sl (noise-white-f32 no))
        (f32slice-copy-from-f32vector ch2-sl (noise-white-f32 no))

        ; advance oscillator
        (osc-fill-slice! o2 ch1-sl)

        ; mixdown
        (mixer-mix-new! x u8)))))

(##sys#gc)

; OLD
; (import boar-param boar-mixer boar-slice srfi-4 srfi-18 (chicken string))
; 
; (define mix (mixer-from-lengths 2 2 4))
; (mixer-master-volume-set!   mix     111.0)
; (mixer-master-balance-set!  mix 0   222.0)
; (mixer-master-balance-set!  mix 1   333.0)
; (mixer-channel-volume-set!  mix 0   444.0)
; (mixer-channel-volume-set!  mix 1   777.0)
; (mixer-channel-balance-set! mix 0 0 555.0)
; (mixer-channel-balance-set! mix 0 1 666.0)
; (mixer-channel-balance-set! mix 1 0 888.0)
; (mixer-channel-balance-set! mix 1 1 999.0)
; (params-new (mixer-params mix))
; (define output (make-u8vector 16 0 #t #f))
; (params-updated? (mixer-params mix))
; (mixer-mix! mix output)
; (mixer-audio mix)
; (vector-ref (mixer-slices mix) 0)
; (f32vector-length (mixer-audio mix))
; (f32slice-to-write (mixer-master-slice mix))
; (f32slice-written (mixer-channel-slice mix 0))
; (f32slice-written (mixer-channel-slice mix 1))
; mix

(import boar-param boar-mixer boar-noise boar-slice boar-osc srfi-4 srfi-18)
(define ppp (params-from-lengths 2 3))
(params-set-linear! ppp 0 0 1.0)
(params-set-linear! ppp 0 1 2.0)
(params-set-linear! ppp 0 2 3.0)
(params-set-linear! ppp 1 0 4.0)
(params-set-linear! ppp 1 1 5.0)
(params-set-linear! ppp 1 2 6.0)
(subf32vector (params-vector ppp) 0 (* 64 3))
(subf32vector (params-vector ppp) (* 64 3) (/ 768 2))
;(params-after-fade-cleanup! ppp)
(params-new ppp)
(params-old ppp)
