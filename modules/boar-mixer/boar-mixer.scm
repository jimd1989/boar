(module boar-mixer
  *
  ...
  (define (mixer-master-balance m b)
    (let ((out-ch (mixer-output-channels m))
  ...
  (define (mixer-master-balance-set! m b n)
    (let ((out-ch (mixer-output-channels m))
          (params (mixer-params m)))
      (if (>= b out-ch)
        (error (conc out-ch " channels; got " (+ 1 b)))
  ...
  (define (mixer-channel-volume m ch)
    (let* ((in-ch (mixer-input-channels m))
  ...
  (define (mixer-channel-volume-set! m ch n)
    (let* ((in-ch (mixer-input-channels m))
           (out-ch (mixer-output-channels m))
           (param-count (+ 1 out-ch))
           (idx (+ param-count (* param-count ch)))
  ...
  (: mixer-channel-balance-set! ((struct mixer) fixnum fixnum float -> noreturn))
  (define (mixer-channel-balance-set! m ch b n)
    (let* ((in-ch (mixer-input-channels m))
           (out-ch (mixer-output-channels m))
           (param-count (+ 1 out-ch))
  ...
  (define (mixer-master-slice m)
    (vector-ref (mixer-slices m) 0))

  (: mixer-channel-slice ((struct mixer) fixnum --> (struct f32slice)))
  (define (mixer-channel-slice m ch)
    (let ((in-ch (mixer-input-channels m)))
      (if (> (+ 1 ch) in-ch)
        (error (conc in-ch " input channels; got " (+ 1 ch)))
        (vector-ref (mixer-slices m) (+ ch 1)))))
